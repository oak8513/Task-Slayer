...
### ☑ Batch 4 — Subtasks + tags + streaks + achievements + pet evolution (~2 hr, ALL gated by flags, default ON) — done 2026-04-24 (split 4A+4B), sha 6f6a3fb, notes: all 5 sub-features complete and flag-gated
**SPLIT:** subtasks + tags done session 4 (sha 9473b3d). Streaks + achievements + pet evolution done session 5 (sha 6f6a3fb).
...
- ☑ Streak counter: new `taskslayer/streak/v1` `{current, best, lastKillDate}`. Render in HUD — done 2026-04-24.
- ☑ Achievements: `taskslayer/achievements/v1` with 12 milestones. Toast on unlock. MEDALS HUD cell + modal — done 2026-04-24.
- ☑ Pet evolution: cyberdog.jsx tier 1/2/3 by level bracket (1-4/5-9/10+), glow + scale + label — done 2026-04-24.
- Flags: `flags.subtasks`, `flags.tags`, `flags.streaks`, `flags.achievements`, `flags.petEvolution` all registered (default ON).

### ☑ Batch 5 — Browser notifications (~1.5 hr, gated, default OFF) — done 2026-04-24, sha 16955f1, notes: nag chip in header, 60s due-crossing interval, notifiedTasksRef prevents duplicates, flag default OFF
